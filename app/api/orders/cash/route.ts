...
export async function POST(req: Request) {
  try {
    ...
    if (!customerName || !customerEmail || !validPhone(phone))
    ...
    const order = await prisma.order.create({
      data: {
        ...
        customerEmail,
        customerPhone: phone,
        fulfillmentType: b.fulfillment.type,
        fulfillmentData: JSON.stringify(b.fulfillment),
        subtotal,
        total: subtotal,
        currency: "INR",
        status: "RECEIVED",
    ...
    console.error(e);
    return NextResponse.json(
      { error: "Could not create cash order." },
      { status: 500 },
    );
  }
}
