...
export async function POST(req: Request) {
  try {
    ...
    const customerEmailInput = String(
      body.customerEmail ?? body.email ?? "",
    ).trim();
    const phone = String(body.customerPhone ?? body.phone ?? "").replace(
      /\D/g,
      "",
    );
    if (!items.length)
      return NextResponse.json({ error: "Cart is empty" }, { status: 400 });
    if (!customerNameInput || !customerEmailInput || !validPhone(phone))
      return NextResponse.json(
        {
          error:
            "Name, email and a valid 10-digit Indian mobile number are required.",
        },
        { status: 400 },
      );
    ...
    const rOrder = await razorpay.orders.create({
      amount: subtotal * 100,
      currency: "INR",
      receipt: `cafe_${Date.now()}`,
      notes: { customerEmail, customerPhone: phone },
    });
    return NextResponse.json({
      orderId: rOrder.id,
      amount: subtotal * 100,
    ...
    console.error(e);
    return NextResponse.json(
      { error: "Unable to create payment order. Check Razorpay keys." },
      { status: 500 },
    );
  }
}
