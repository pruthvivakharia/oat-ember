...
export async function POST(req: Request) {
  try {
    ...
    if (!b.razorpay_signature || expected !== b.razorpay_signature) return NextResponse.json({ error: 'Invalid payment signature' }, { status: 400 });
    ...
    if (!customerName || !validPhone(phone) || !customerEmail) return NextResponse.json({ error: 'Name, email and a valid 10-digit phone number are required.' }, { status: 400 });
    ...
    const orderItems = items.map((i: any) => {
      ...
      if (!p || !Number.isInteger(quantity) || quantity < 1 || quantity > 10 || shots < 0 || shots > 4) throw new Error('Product unavailable or invalid quantity');
    ...
    if (!fulfillment?.type) return NextResponse.json({ error: 'Choose a handover method first.' }, { status: 400 });
    const order = await prisma.order.create({ data: { userId: (session?.user as any)?.id || null, customerName: customerName || session?.user?.name || 'Guest', customerEmail, customerPhone: phone, fulfillmentType: fulfillment.type, fulfillmentData: JSON.stringify(fulfillment), subtotal, total: subtotal, currency: 'INR', status: 'RECEIVED', paymentMethod: 'RAZORPAY', paymentStatus: 'PAID', razorpayOrderId: b.razorpay_order_id, razorpayPaymentId: b.razorpay_payment_id, razorpaySignature: b.razorpay_signature, items: { create: orderItems } } });
    ...
    try { await sendOrderReceipt({ orderId: order.id, customerName: order.customerName, customerEmail: order.customerEmail, customerPhone: order.customerPhone, total: order.total, paymentMethod: 'RAZORPAY', fulfillmentType: order.fulfillmentType, items: orderItems }); } catch (emailError) { console.error('Receipt email failed', emailError); }
